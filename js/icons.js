/**
 * icons.js — کتابخانه آیکون‌های داخلی پروژه
 * تمام آیکون‌های استفاده شده در برنامه به‌صورت SVG خام و دستی نوشته شده‌اند
 * (بدون وابستگی به فایل‌های خارجی) تا در هر دو تم روشن/تاریک با currentColor
 * هماهنگ شوند و بتوان به‌راحتی به آن‌ها انیمیشن اضافه کرد.
 */
const ICONS = {
    // لوگوی برنامه — آیکون اصلی پروژه (بازگردانی‌شده طبق درخواست)
    ...
    // عنوان بخش سوالات نشان‌شده — نشان پرشده بزرگ
    ...
    bookmarkAdd: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6.3 3.7C6.3 2.76 7.06 2 8 2H14C14.94 2 15.7 2.76 15.7 3.7V20.2L11 17L6.3 20.2V3.7Z" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/>
        <path d="M19 8.2V13.2M16.5 10.7H21.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>
    </svg>`,

    // نشان‌شده (پر)
    bookmarkAdded: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6.3 3.7C6.3 2.76 7.06 2 8 2H14C14.94 2 15.7 2.76 15.7 3.7V20.2L11 17L6.3 20.2V3.7Z" fill="currentColor"/>
        <path d="M16.6 8.6L18.3 10.3L21.8 6.4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`,
    ...
    // شورون برای دکمه چیدمان
    chevronLeft: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M15 6L9 12L15 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`,

    // شورون رو به پایین برای منوی کشویی
    chevronDown: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6 9L12 15L18 9" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`,

    // چیدمان تک‌ستونی
    layout1: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="4" y="4" width="16" height="16" rx="2.6" stroke="currentColor" stroke-width="1.7"/>
    </svg>`,

    // چیدمان دوستونی
    layout2: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect x="3.4" y="4" width="7.6" height="16" rx="2.1" stroke="currentColor" stroke-width="1.7"/>
        <rect x="13" y="4" width="7.6" height="16" rx="2.1" stroke="currentColor" stroke-width="1.7"/>
    </svg>`,
...
/**
 * برگرداندن رشته SVG یک آیکون به همراه کلاس دلخواه
 * @param {string} name نام آیکون در شیء ICONS
 * @param {string} extraClass کلاس(های) اضافه برای svg
 */
function getIcon(name, extraClass = '') {
    const raw = ICONS[name];
    if (!raw) return '';
    const cls = `icon${extraClass ? ' ' + extraClass : ''}`;
    // تزریق کلاس به تگ svg
    return raw.replace('<svg ', `<svg class="${cls}" `);
}

/**
 * تمام عناصر دارای data-icon را با SVG واقعی جایگزین می‌کند.
 * برای آیکون‌های ثابتِ نوشته‌شده در HTML استفاده می‌شود.
 */
